document.addEventListener("DOMContentLoaded", () => {
    ...
    document.querySelector('.popup__close').addEventListener('click', function(event) {
        ...
        document.querySelector('#popup').style.display = 'none'; // Закрытие всплывающего окна
    });
    ...
    //         // Показать popup после добавления блюда
    //         showPopup();
    ...
    function handleDishClick(event) {
        ...
        if (dish) {
            ...
            selectedDishes[category] = dish;
            console.log(`Добавлено блюдо: ${dish.name} в категорию ${category}`);
            const totalOrderValue = updateOrder();  // Вычисляем и получаем итоговую сумму
            // try {
            //     const parsedDish = JSON.parse(dish);
                 window.localStorage.setItem(`load_dish_${dish.name}`, dish.name);
            // } catch (e) {
            //     console.error('Ошибка парсинга JSON:', e);
            // }
    
            // Показ popup и обновление суммы внутри popup
            showPopup(); 
            document.getElementById('order-total').textContent = `${totalOrderValue} ₽`; // Вставляем сумму в popup
        } else {
            console.error('Блюдо не найдено:', keyword);
        }
    }
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (closeButton) {
            closeButton.addEventListener('click', (event) => {
                event.preventDefault(); // Предотвращаем любое действие по умолчанию для ссылки
