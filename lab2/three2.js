...
/////////////////////
            ...
            row.innerHTML = `
                ...
                <td>
                    <span class="action-buttons">
                        <button class="view" title="Просмотреть" data-order-id="${order.id}">&#128065;</button>
                        <button class="edit" title="Редактировать" data-order-id="${order.id}">&#9998;</button>
                        <button class="delete" title="Удалить" data-order-id="${order.id}">&#128465;</button>
                    </span>
                </td>
            `;
    ...
    function openViewModal(orderId) {
        ...
        document.getElementById('viewOrderDelivery').innerHTML = `
            ...
            <p>Время доставки: ${order.delivery_type === 'now' ? 'Сейчас' : order.delivery_time}</p>
        ...
        document.getElementById('viewOrderItems').innerHTML = [
            ...
            getDishName('drink', order.drink_id),
            getDishName('dessert', order.dessert_id)
        ].filter(dish => dish).map(item => `<p>${item}</p>`).join('');
